...
VLU_EXP_PSACL_COSNT:          PSACL_int                         {Value* value= new Value; value->base_type = TY_INTEGER; value->val.integer_value = $1; $$ = new ConstantExp(value); ((Exp*)$$)->return_type = new Type(TY_INTEGER);} // ConstantExp可能有转化问题
                    | PSACL_realnum                            {Value* value= new Value; value->base_type = TY_REAL; value->val.real_value = atof(PASCALiconTable[$1].id); $$ = new ConstantExp(value); ((Exp*)$$)->return_type = new Type(TY_REAL);}
                    | PSACL_character                            {Value* value= new Value; value->base_type = TY_CHAR; value->val.char_value = PASCALiconTable[$1].id[0]; $$ = new ConstantExp(value); ((Exp*)$$)->return_type = new Type(TY_CHAR);}
                    | PSACL_str                          {Value* value= new Value; value->base_type = TY_STRING; value->val.string_value = new string(PASCALiconTable[$1].id); $$ = new ConstantExp(value); ((Exp*)$$)->return_type = new Type(TY_STRING);}
                    | EXP_SYSTEM_CONTS                           {$$ = $1;}
                    ;

EXP_SYSTEM_CONTS:              TOKEN_TRUE                            {Value* value= new Value; value->base_type = TY_BOOLEAN; value->val.boolean_value = true; $$ = new ConstantExp(value); ((ConstantExp*)$$)->return_type = new Type(TY_BOOLEAN);}
...
Category_TYPE_LIST_DECLARTION:      TOKEN_ARRAY TOKEN_LB Category_TYPE_DECLARTION_EASY TOKEN_RB TOKEN_OF Category_TYPE_DECLARTION         {$$ = $3; $$->child_type.push_back($6);}
                    ;

Category_REC_DECLARTION:     TOKEN_RECORD TYVAL_FIE_ARRAY_dec TOKEN_END    {$$ = new Type(TY_RECORD); $$->child_type = *$2;} // TODO TY_ROCORD type
                    ;
...
TYVAL_FIE_PASCAL_dec:           STR_designate_array TOKEN_COLON Category_TYPE_DECLARTION TOKEN_SEMI {$$ = new std::vector<Type *>(); for(auto iter : *$1) {Type * tmp = new Type(); tmp->name = iter; tmp->child_type.push_back($3); $$->push_back(tmp);}} // std::vector<Type *>
                ;   
...
STATEMENT_PASCL_FOR:             TOKEN_FOR PSACL_identify TOKEN_ASSIGN EXP_PASCL_express Category_DIR_PAS EXP_PASCL_express TOKEN_DO STATEMENT_PASCAL {$$ = new ForStm(PASCALiconTable[$2].id, $4, $6, $5->base_type); ((ForStm*)$$)->addLoop($8);}
